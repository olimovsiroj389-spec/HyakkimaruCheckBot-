(() => {
  ...
  const screens = ["intro","already","quiz","thinking","result"];
  ...
  function parseOldScores(value){
    if(!value) return null;
    try{
      const parsed = JSON.parse(value);
      const safe = {};
      ["gryffindor","slytherin","ravenclaw","hufflepuff"].forEach(k => {
        const n = Number(parsed?.[k] ?? 0);
        safe[k] = Number.isFinite(n) ? Math.max(0, Math.min(1000, Math.trunc(n))) : 0;
      });
      return safe;
    }catch{ return null; }
  }
  ...
  function showAlreadyTaken(){
    const q = params();
    if(q.get("taken") !== "1") return false;
    const faculty = q.get("faculty");
    if(!facultyInfo[faculty]) return false;
    const f = facultyInfo[faculty];
    selectedFaculty = faculty;
    $("oldResultEmoji").textContent = f.emoji;
    $("oldResultName").textContent = f.name;
    $("oldResultTitle").textContent = f.title;
    $("already").querySelector(".lead").textContent = "Siz ushbu testda allaqachon qatnashgansiz. Avvalgi natijangiz saqlangan.";

    const oldScores = parseOldScores(q.get("scores"));
    const box = $("oldScoreBox");
    box.innerHTML = "";
    if(oldScores){
      ["gryffindor","slytherin","ravenclaw","hufflepuff"].forEach(k => {
        const row=document.createElement("div");
        row.className="score-row";
        row.innerHTML=`<span>${facultyInfo[k].emoji} ${facultyInfo[k].name}</span><strong>${oldScores[k]}</strong>`;
        box.appendChild(row);
      });
    } else {
    ...
    // Slytherin linki bot tomonidan natijadan keyin yuboriladi; bu sahifada boshqa link chiqarilmaydi.
    if(faculty === "slytherin"){
      $("oldSlytherinBox").style.display = "block";
      $("oldSlytherinText").textContent = "Guruhga qo‘shilish havolasi natijadan keyin bot xabarida berilgan.";
    }
    show("already");
    return true;
  }
  ...
  $("againBtn").addEventListener("click", () => {
    // Bir martalik test: qayta boshlash tugmasi endi ishlatilmaydi.
    if (tg?.showAlert) tg.showAlert("Bu testda avval qatnashish uchun natijani yuboring. Har bir foydalanuvchi faqat bir marta qatnashadi.");
  });
